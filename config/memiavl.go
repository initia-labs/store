...
const (
	...
	flagMemIAVLHistoricalQueryLimit = "memiavl.historical-query-limit"
)
...
type MemIAVLConfig struct {
	...
	// HistoricalQueryLimit limits how many recent versions are kept in the in-memory historical query cache.
	HistoricalQueryLimit int `mapstructure:"historical-query-limit"`
}
...
func DefaultMemIAVLConfig() MemIAVLConfig {
	return MemIAVLConfig{
		...
		HistoricalQueryLimit: DefaultHistoricalQueryLimit,
	}
}
...
func GetMemIAVLConfig(appOpts servertypes.AppOptions) MemIAVLConfig {
	return MemIAVLConfig{
		...
		HistoricalQueryLimit: cast.ToInt(appOpts.Get(flagMemIAVLHistoricalQueryLimit)),
	}
}
...
func AddMemIAVLConfigFlags(startCmd *cobra.Command) {
	...
	startCmd.Flags().Int(flagMemIAVLHistoricalQueryLimit, DefaultHistoricalQueryLimit, "Number of recent versions to retain in the in-memory historical query cache (0 follows snapshot interval)")
}
...
# HistoricalQueryLimit defines how many recent versions are retained for in-memory historical queries.
# Values greater than snapshot-interval are clamped to snapshot-interval. Set to 0 to follow snapshot-interval.
historical-query-limit = {{ .MemIAVL.HistoricalQueryLimit }}
`
