...
func NewStore(tree *Tree, logger log.Logger) *Store {
	return &Store{tree: tree, logger: logger}
}

func (st *Store) SetTree(tree *Tree) {
	st.tree = tree
}
...
func (st *Store) Set(key, value []byte) {
	st.changeSet.Pairs = append(st.changeSet.Pairs, &KVPair{
		Key: key, Value: value,
	})
}
...
func (st *Store) PopChangeSet() ChangeSet {
	cs := st.changeSet
...
func (st *Store) Query(req *types.RequestQuery) (res *types.ResponseQuery, err error) {
	...
	case "/subspace":
		...
		for ; iterator.Valid(); iterator.Next() {
			pairs.Pairs = append(pairs.Pairs, Pair{Key: iterator.Key(), Value: iterator.Value()})
		}
		err := iterator.Close()
		if err != nil {
...
func getProofFromTree(tree *Tree, key []byte, exists bool) *cmtprotocrypto.ProofOps {
	var (
