...
// NewStore creates a new versiondb store
...
// NewStoreAtVersion opens the versiondb at a specific version, trimming any later versions.
func NewStoreAtVersion(dir string, version int64) (Store, error) {
	db, cfHandle, err := OpenVersionDBAndTrimHistory(dir, version)
	if err != nil {
		return Store{}, err
	}
	return Store{
...
// NewStoreWithDB creates a new versiondb store with existing rocksdb instance and column family handle
...
func (s Store) FeedChangeSet(version int64, storeKey string, changeSet *iavl.ChangeSet) error {
	var ts [TimestampSize]byte
	binary.LittleEndian.PutUint64(ts[:], uint64(version))
